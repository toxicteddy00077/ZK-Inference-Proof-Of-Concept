...
##1. Client gives input to model -> model runs first lightweight layers on client machine and encrypts the resulting weights
##2. Client sends encrypted weights and a proof of valid inputs to the server with verification key using gRPC.
##3. The server recives the weights and verifies the proof before proceeding. If the porrf is valid, I,e the input is of an accpetbale field, the server runs the remaining and compuataiontally intesive part of inference
    and sends back the ouput with another proof that the weights sent by the client were only used for inference.
##4. The client then recives the proof and the response, and is thus able to ascertain no malicious activity has taken place with the data.
